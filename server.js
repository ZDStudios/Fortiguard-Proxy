...
function setCookie(res, value) {
  res.setHeader("Set-Cookie", `fp_auth=${encodeURIComponent(value)}; HttpOnly; Path=/; Max-Age=86400; SameSite=Lax`);
}
...
body{background:#08080f;color:#aaaacc;font-family:Consolas,'Courier New',monospace;min-height:100vh;display:flex;align-items:center;justify-content:center}
...
input{width:100%;background:#06060e;border:1px solid #1e1e3a;border-radius:8px;padding:13px 16px;color:#aaaacc;font-family:Consolas,monospace;font-size:.95em;outline:none;margin-bottom:12px;letter-spacing:2px}
input:focus{border-color:#00d4ff}
button{width:100%;background:#005c2e;border:none;border-radius:8px;padding:13px;color:#00ff88;font-family:Consolas,monospace;font-size:.95em;font-weight:bold;letter-spacing:2px;cursor:pointer}
...
<div class="card">
  ...
  ${err ? '<div class="err">Invalid token — try again</div>' : ''}
</div>
</body>
</html>`;
}
...
.btn-sm{background:transparent;border:1px solid #1e1e3a;border-radius:6px;padding:4px 12px;color:#444466;font-family:Consolas,monospace;font-size:.75em;cursor:pointer;text-decoration:none}
...
<div class="stats">
  <div class="stat"><div class="sv" id="s-online">-</div><div class="sl">ONLINE NOW</div></div>
  <div class="stat"><div class="sv" id="s-total">-</div><div class="sl">KNOWN DEVICES</div></div>
  <div class="stat"><div class="sv" id="s-blocked">-</div><div class="sl">BLOCKED</div></div>
  <div class="stat"><div class="sv" id="s-tunnels">-</div><div class="sl">ACTIVE TUNNELS</div></div>
</div>
<div class="wrap">
...
function blockDevice(name){
  ...
  fetch('/api/block',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device:name})})
  .then(function(){refresh();}).catch(function(){});
}
function unblockDevice(name){
  fetch('/api/unblock',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device:name})})
  .then(function(){refresh();}).catch(function(){});
}
function refresh(){
  fetch('/api/devices')
  ...
  .then(function(data){
    ...
    data.sort(function(a,b){
      var ao=(now-new Date(a.lastSeen))<THRESH&&!a.blocked;
      var bo=(now-new Date(b.lastSeen))<THRESH&&!b.blocked;
      if(ao!==bo) return bo-ao;
      return new Date(b.lastSeen)-new Date(a.lastSeen);
    });
    tb.innerHTML='';
    data.forEach(function(d){
      var isOnline=(now-new Date(d.lastSeen))<THRESH&&!d.blocked;
      var st,act;
      if(d.blocked){
        st='<span class="dot-r">&#9679;</span><span class="badge badge-blocked">BLOCKED</span>';
        act='<button class="unblock-btn" onclick="unblockDevice(\''+esc(d.device)+'\')">UNBLOCK</button>';
      } else if(isOnline){
        st='<span class="dot-g blink">&#9679;</span><span class="badge badge-live">ONLINE</span>';
        act='<button class="block-btn" onclick="blockDevice(\''+esc(d.device)+'\')">BLOCK</button>';
      } else {
...
wss.on("connection", (ws, req) => {
  ...
  if (path === "/tunnel") {
    ...
    const cleanup = () => { info.activeTunnels = Math.max(0, (info.activeTunnels || 1) - 1); };
